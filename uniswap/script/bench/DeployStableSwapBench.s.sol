...
/// @notice The Solidity half of the StableSwap benchmark: a v4 deployment and `StableSwapHook.sol`.
///
/// Its Rust twin cannot be deployed from here — it needs a CREATE2 salt mined against its WASM init
/// code, and forge cannot call it afterwards either. `bench-stableswap.bash` does that part.
contract DeployStableSwapBenchScript is Script {
    ...
    StableSwapHook internal solidityHook;

    function _deployStableSwapHook() private {
        uint160 flags = uint160(Hooks.BEFORE_SWAP_FLAG);
        address create2Deployer = vm.envAddress("CREATE2_DEPLOYER");

        (address solidityAddress, bytes32 soliditySalt) =
            HookMiner.find(create2Deployer, flags, type(StableSwapHook).creationCode, abi.encode(poolManager));
        solidityHook = new StableSwapHook{salt: soliditySalt}(poolManager);
        require(address(solidityHook) == solidityAddress, "stableswap hook address mismatch");
    }
    ...
    function run() public {
        ...
        console.log("StableSwap (sol) :", address(solidityHook));
    }
}
