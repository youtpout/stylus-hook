...
/// @notice A v4 deployment fronted by OpenZeppelin's `AntiSandwichHook`, to price what it does.
///
/// The hook checkpoints the pool at the top of each block and, for `zeroForOne == false` swaps,
/// replays `Pool.swap` against that checkpoint to cap the price at its beginning-of-block value.
/// `zeroForOne == true` swaps skip the replay entirely, so the gap between the two directions is
/// the simulation, isolated.
contract DeployAntiSandwichBenchScript is Script {
    ...
    ArbAntiSandwichMock internal solidityHook;
    ...
    function _deployComputeHook() private {
        ...
        (address solidityAddress, bytes32 soliditySalt) =
            HookMiner.find(create2Deployer, flags, type(ArbAntiSandwichMock).creationCode, abi.encode(poolManager));
        solidityHook = new ArbAntiSandwichMock{salt: soliditySalt}(poolManager);
        require(address(solidityHook) == solidityAddress, "anti-sandwich hook address mismatch");
    }
    ...
    function run() public {
        ...
        console.log("AntiSandwich     :", address(solidityHook));
    }
}
