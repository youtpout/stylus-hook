...
import {OracleImplementation} from "./shared/implementation/OracleImplementation.sol";
...
contract TestOracle is Test, GasSnapshot {
    ...
    function setUp() public {
        ...
        initializedOracle.initialize(OracleImplementation.InitializeParams({time: 0, tick: 0, liquidity: 0}));
    }
    ...
    function testGrowAfterWrap() public {
        ...
        // index is now 1
        initializedOracle.update(OracleImplementation.UpdateParams({advanceTimeBy: 2, liquidity: 1, tick: 1}));
    ...
    function testWrite() public {
        initializedOracle.update(OracleImplementation.UpdateParams({advanceTimeBy: 1, tick: 2, liquidity: 5}));
        assertEq(initializedOracle.index(), 0);
        assertObservation(
            initializedOracle,
            0,
        ...
        initializedOracle.update(OracleImplementation.UpdateParams({advanceTimeBy: 5, tick: -1, liquidity: 8}));
        assertEq(initializedOracle.index(), 0);
        assertObservation(
            initializedOracle,
            0,
        ...
        initializedOracle.update(OracleImplementation.UpdateParams({advanceTimeBy: 3, tick: 2, liquidity: 3}));
        assertEq(initializedOracle.index(), 0);
        assertObservation(
            initializedOracle,
            0,
    ...
    function testWriteAddsNothingIfTimeUnchanged() public {
        initializedOracle.grow(2);
        initializedOracle.update(OracleImplementation.UpdateParams({advanceTimeBy: 1, tick: 3, liquidity: 2}));
        assertEq(initializedOracle.index(), 1);
        initializedOracle.update(OracleImplementation.UpdateParams({advanceTimeBy: 0, tick: -5, liquidity: 9}));
        assertEq(initializedOracle.index(), 1);
    }

    function testWriteTimeChanged() public {
        initializedOracle.grow(3);
        initializedOracle.update(OracleImplementation.UpdateParams({advanceTimeBy: 6, tick: 3, liquidity: 2}));
        assertEq(initializedOracle.index(), 1);
        initializedOracle.update(OracleImplementation.UpdateParams({advanceTimeBy: 4, tick: -5, liquidity: 9}));
    ...
    function testWriteGrowsCardinalityWritingPast() public {
        ...
        assertEq(initializedOracle.cardinality(), 1);
        initializedOracle.update(OracleImplementation.UpdateParams({advanceTimeBy: 3, tick: 5, liquidity: 6}));
        assertEq(initializedOracle.cardinality(), 4);
        initializedOracle.update(OracleImplementation.UpdateParams({advanceTimeBy: 4, tick: 6, liquidity: 4}));
        assertEq(initializedOracle.cardinality(), 4);
        assertEq(initializedOracle.index(), 2);
        assertObservation(
            initializedOracle,
    ...
    function testWriteWrapsAround() public {
        initializedOracle.grow(3);
        initializedOracle.update(OracleImplementation.UpdateParams({advanceTimeBy: 3, tick: 1, liquidity: 2}));

        initializedOracle.update(OracleImplementation.UpdateParams({advanceTimeBy: 4, tick: 2, liquidity: 3}));

        initializedOracle.update(OracleImplementation.UpdateParams({advanceTimeBy: 5, tick: 3, liquidity: 4}));
    ...
    function testWriteAccumulatesLiquidity() public {
        initializedOracle.grow(4);
        initializedOracle.update(OracleImplementation.UpdateParams({advanceTimeBy: 3, tick: 3, liquidity: 2}));

        initializedOracle.update(OracleImplementation.UpdateParams({advanceTimeBy: 4, tick: -7, liquidity: 6}));

        initializedOracle.update(OracleImplementation.UpdateParams({advanceTimeBy: 5, tick: -2, liquidity: 4}));
    ...
    function testObserveFailsIfOlderDoesNotExist() public {
        oracle.initialize(OracleImplementation.InitializeParams({liquidity: 4, tick: 2, time: 5}));
        uint32[] memory secondsAgos = new uint32[](1);
        secondsAgos[0] = 1;
    ...
    function testDoesNotFailAcrossOverflowBoundary() public {
        oracle.initialize(OracleImplementation.InitializeParams({liquidity: 4, tick: 2, time: 2 ** 32 - 1}));
    ...
    function testInterpolationMaxLiquidity() public {
        oracle.initialize(OracleImplementation.InitializeParams({liquidity: type(uint128).max, tick: 0, time: 0}));
        oracle.grow(2);
        oracle.update(OracleImplementation.UpdateParams({advanceTimeBy: 13, tick: 0, liquidity: 0}));
    ...
    function testInterpolatesSame0And1Liquidity() public {
        oracle.initialize(OracleImplementation.InitializeParams({liquidity: 1, tick: 0, time: 0}));
        oracle.grow(2);
        oracle.update(OracleImplementation.UpdateParams({advanceTimeBy: 13, tick: 0, liquidity: type(uint128).max}));
        (, uint160 secondsPerLiquidityCumulativeX128) = observeSingle(oracle, 0);
    ...
    function testInterpolatesAcrossChunkBoundaries() public {
        oracle.initialize(OracleImplementation.InitializeParams({liquidity: 0, tick: 0, time: 0}));
        oracle.grow(2);
        oracle.update(OracleImplementation.UpdateParams({advanceTimeBy: 2 ** 32 - 6, tick: 0, liquidity: 0}));
        (, uint160 secondsPerLiquidityCumulativeX128) = observeSingle(oracle, 0);
    ...
    function setupOracleWithManyObservations(uint32 startingTime) internal {
        oracle.initialize(OracleImplementation.InitializeParams({liquidity: 5, tick: -5, time: startingTime}));
        oracle.grow(5);
        oracle.update(OracleImplementation.UpdateParams({advanceTimeBy: 3, tick: 1, liquidity: 2}));
        oracle.update(OracleImplementation.UpdateParams({advanceTimeBy: 2, tick: -6, liquidity: 4}));
        oracle.update(OracleImplementation.UpdateParams({advanceTimeBy: 4, tick: -2, liquidity: 4}));
        oracle.update(OracleImplementation.UpdateParams({advanceTimeBy: 1, tick: -2, liquidity: 9}));
        oracle.update(OracleImplementation.UpdateParams({advanceTimeBy: 3, tick: 4, liquidity: 2}));
        oracle.update(OracleImplementation.UpdateParams({advanceTimeBy: 6, tick: 6, liquidity: 7}));
    }
